...
df = "DESeq2/DESeq2_Group_SDRPL_vs_HCLv.xlsx" |>
    readxl::read_xlsx() |>
    setDT()
...
gr = ggplot(data = df) +
  ...
  geom_text_repel(
    data = df2, aes(x = log2FoldChange, y = -log10(padj), label = gene_name),
...
ggsave(
  plot = gr, filename = paste0("volcano_SDRPL_vs_HCLv_padj.jpeg"), 
